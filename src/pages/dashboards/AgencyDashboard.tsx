...
//     const response = await fetch(`${API_BASE_URL}/messaging/active-dsps`, {
//       method: "GET",
//       headers: { Authorization: `Bearer ${token}` },
//     });
    
//     if (!response.ok) throw new Error('Failed to fetch active DSPs');
...
//       conversation = await startShiftConversation(token, shiftId, dspId);
//     } else {
//       // Start general conversation
//       conversation = await startConversation(token, dspId);
//     }
...
//       const [statsData, shiftsData, applicationsData, analyticsData, complianceData] = await Promise.all([
...
//   const handleMessageDSP = async (shiftId: string, dspId: string) => {
//     try {
//       setError('');
//       const token = getToken();
//       if (!token) {
...
//       const conversation = await startShiftConversation(token, shiftId, dspId);
...
//       toast({
//         title: "Conversation started",
//         description: "You can now message the DSP about this shift.",
//       });
...
//         rate: parseFloat(shiftData.hourlyRate.replace('$', '')),
...
import { 
  ...
  getShiftConversations,
  startShiftConversation,
  startConversation, // Add this import
  getActiveDSPs // Add this import
} from '@/service/data';
